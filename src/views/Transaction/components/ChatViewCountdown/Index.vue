<template>
  <div class="countdown">
    <p class="countdown-text">
      交易倒计时：<van-count-down ref="countDown" @finish="finishTime" :time="countDown(itemData.time)" />
    </p>
    <div v-if="isReceive" :class="`countdown-btns ${disabledBtn ? 'disabled' : ''}`">
      <span class="countdown-btns_y" @click="handleClickYes">我收到钱</span>
      <span class="countdown-btns_n" @click="handleClickNo">我没收到钱</span>
    </div>
    <div v-else :class="`countdown-btns ${disabledComplaintBtn ? 'disabled' : ''}`">
      ...
      return !!array.find((item) => {
        if (item.extra) {
          if ("ender_oder" in item.extra) {
            ...
            console.log("时间停止", this.$refs.countDown);
