<template>
  <div class="page password">
    <common-title title="校验密保问题" isBack />
    ...
    <div class="password-btn">
      <div class="password-btn_cancel" @click="handleCancel">
        取消
      </div>
      <div
        :class="btnDiseable ? 'password-btn_diseable' : 'password-btn_confirm'"
        @click="handleSubimt"
      >
        确定
      </div>
...
import { findPasswordByProtected } from "@/api/user";
...
export default {
  ...
  methods: {
    ...
    handleSubimt() {
      ...
      findPasswordByProtected(params).then(res => {
        this.$router.replace({
          path: "/user/reset_password",
          query: {
            uid: res.uid,
            password_token: res.password_token,
            time: res.time,
          }
        });
      });
